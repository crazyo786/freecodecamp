...
#  MESSAGES AND REFER " self.modified_database.sh " FILE TO IDENTIFY THE MODIFICATION 	 #
#				DONE IN THIS SCRIPT            				 #
# 											 #
# ************************************************************************************** #
...
if ! $($PSQL "CREATE TABLE appointments(appointment_id SERIAL PRIMARY KEY, customer_id INT REFERENCES customers(customer_id), service_id INT REFERENCES services(service_id),time VARCHAR(50))" &> /dev/null)
then
	sleep 1
